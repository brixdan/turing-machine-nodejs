...
function buildDivGroup(arr = ["0"], size = 10, depth = 500, aheads = 2) {
  let nuvo = [0],
    t,
    nuvoStr;
  let i = 0;
  ...
  while (i < size * 2) {
    ...
    if (t === 1) {
      arr.push(nuvoStr);
    } // nuvo fails, log the cause
    else res[i] = "!!!Failed nuvo = " + nuvoStr + " caused by " + t;
  }
...
memo["champ"] = memo["champ"] ?? { steCps: 0, tape: "" };

let divGroup = buildDivGroup(["0", "1"], 40, (depth = 200), (aheads = 10)); // base1_l10.txt
let memoSorted = Object.keys(memo)
  .sort(Array.compareTapes)
  .map((val) => val + ":" + memo[val]);
memo.storeData(memo, "../../_data/base1_size40_depth200.json");
memo.storeData(memoSorted, "../../_data/sorted-base1_size40_depth200.json");
divGroup["champ"] = memo["champ"];
memo.storeData(divGroup, "../../_data/divGroup-base1_size40_depth200.json");
